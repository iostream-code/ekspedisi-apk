// bump-version.cjs -- adaptasi dari pola yang sama persis dipakai finance-apk/
// admin-finance-apk (lihat bump-version.js di sana). CommonJS (.cjs, bukan
// .js) SENGAJA -- package.json app ini "type": "module", `require()` polos
// di file .js bakal gagal, .cjs override itu terlepas dari "type" package.json.
//
// Beda dari finance-apk: app ini Vite (ESM), bukan Cordova <script> tag polos
// -- generate `src/js/app-version.js` sbg ES module (`export const`, bukan
// `var` global) supaya bisa di-import biasa oleh versionCheck.js & ikut
...
// Dibaca oleh versionCheck.js (src/js/) -- android-versionCode (integer,
// naik 1 tiap rilis) yang dikirim ke POST /config/check-version, dibandingkan
// `>=` ke config_value_minimal (tabel `config`, config_id='VERSION_EKSPEDISI_PUSAT',
// lihat ekspedisi-apk-backend). Integer dipilih (bukan version string) krn
// itu tujuannya: gampang dibandingkan, konsisten dgn sibling apps lain di
// workspace ini (finance-apk, admin-finance-apk).
function updateAppVersionJs(androidCode, versionString) {
  const outPath = path.join(__dirname, 'src', 'js', 'app-version.js');
  const content = `// [BERKAS AUTO-GENERATE oleh bump-version.cjs -- JANGAN DIEDIT MANUAL]
// Dibaca oleh versionCheck.js saat app bootstrap -- dikirim ke
// POST /config/check-version supaya backend bisa bandingkan dgn
// config_value_minimal (tabel config, config_id='VERSION_EKSPEDISI_PUSAT').
...
function bumpVersion(type = 'patch', customVersion = null) {
  ...
  console.log('src/js/app-version.js berhasil diupdate\n');

  console.log(`PENTING: nilai di DB (tabel config, config_id='VERSION_EKSPEDISI_PUSAT',`);
